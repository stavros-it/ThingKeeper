...
def size_to_available(widget: QWidget, w_ratio: float, h_ratio: float) -> None:
    """Resize ``widget`` to a fraction of the available screen area.

    ``w_ratio`` and ``h_ratio`` are clamped to ``[0.0, 1.0]``.  The
    widget is centred within the available geometry of its current
    screen (or the primary screen if it has none yet).  Falls back
    silently when no screen is available (headless tests).
    """
